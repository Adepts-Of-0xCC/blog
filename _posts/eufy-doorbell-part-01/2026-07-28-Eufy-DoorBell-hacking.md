...
tags: [red team, research, eufy, hardware hacking, doorbell, OCEAN, hidden wifi, firmware, soundwave, X-C3LL]
description: Analyzing Eufy Security Ecosystem and reverse engineering of its doorbell component
image: 
---
...
- [0x00 Preamble](#0x00-preamble). Introduction about how this research started. You can skip it freely if you are only interested in technical details.
- [0x01 Introduction to the ecosystem](#0x01-introduction-to-the-ecosystem). Brief explanation about the product, its components and how they are related.
- [0x02 Jamming](#0x02-jamming). You can remotely disconnect the doorbell from the "management" network avoiding it to stream video/audio. A crappy Proof of Concept is provided.
- [0x03 Soundwave sync protocol](#0x03-soundwave-sync-protocol). Reverse Engineering the soundwave protocol used to sync the doorbell with the homebase. 
- [0x04 Extracting and decrypting OCEAN_XXXXXX creds from memory dump](#0x04-extracting-and-decrypting-ocean_xxxxxx-creds-from-flash-memory). Recovered and revere enginering of the encrypted configuration file that contains the credentials used by the doorbell to connect to the hidden network.
...
So I did the obvious thing... figure out the most common model used by them and try to pwn it __:)__
